...
contract Crowdsale is Ownable {
    ...
    uint256 public startTime = 1520071200; // 03/03/2018 @ 10:00am (UTC)
    ...
    function getTimeBonus() public view returns (uint256) {
        if(now < startTime + 1 days) { // Round X
            ...
            return timeBonus3;
        } else if(now >= startTime + 15 days && now < endTime) { // Round 2
    ...
    */
    ...
    function buyTokens(address beneficiary) saleIsOn public payable {
        ...
        TokenPurchase(msg.sender, beneficiary, weiAmount, tokensSumBonus);

        wallet.transfer(msg.value);

        uint256 taaTokens = tokensSumBonus.mul(20).div(all);
